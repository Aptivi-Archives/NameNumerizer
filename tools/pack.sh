...
# Pack binary
echo Packing binary...
cd "../NameNumerizer.Core/bin/$releaseconf/netstandard2.0/" && "$zippath" -r /tmp/$version-bin.zip . && cd -
cd "../NameNumerizer/bin/$releaseconf/net8.0/" && "$zippath" -r /tmp/$version-demo.zip . && cd -
if [ ! $? == 0 ]; then
